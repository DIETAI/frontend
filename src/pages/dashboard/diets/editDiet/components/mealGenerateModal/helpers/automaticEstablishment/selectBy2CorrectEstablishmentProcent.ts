...
export const selectByCorrect2EstablishmentProcent = (
  cartesianGroups: ICartesianResult[],
  // mealEstablishment: TMealEstablishment | undefined
  mealEstablishment?: any
) => {
  const automatic2CorrectEstablishmentGroup: IAutomaticEstablishmentGroupInfo =
    {
      report: [],
      group: undefined,
      type: "automaticEstablishmentCorrect2Macro",
      name: "wybieranie zestawu dla wartości % 2 makroskładników mieszczących się w określonym przedziale",
      description:
        "wybieranie zestawu dla wartości % 2 makroskładników mieszczących się w określonym przedziale",
    };
  ...
  if (selectedGroupsByPerfectKcal.length < 1) {
    return automatic2CorrectEstablishmentGroup;
  }
  ...
  const macroEstablishmentProcent = selectedGroupsByPerfectKcal.map((group) => {
    ...
    if (filterMacroCheckArr.length !== 2) {
      return {
        ...group,
  ...
  console.log({ automatic2MacroEstGroups: selectedCorrectAutomaticGroups });

  if (selectedCorrectAutomaticGroups.length < 1) {
    return {
      ...automatic2CorrectEstablishmentGroup,
