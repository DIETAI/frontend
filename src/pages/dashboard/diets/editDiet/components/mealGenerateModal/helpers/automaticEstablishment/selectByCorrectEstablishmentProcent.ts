...
export const selectByCorrectEstablishmentProcent = (
  cartesianGroups: ICartesianResult[],
  // mealEstablishment: TMealEstablishment | undefined
  mealEstablishment?: any
) => {
  const automaticEstablishmentGroup: IAutomaticEstablishmentGroupInfo = {
    report: [],
    group: undefined,
    type: "automaticEstablishment",
    name: "wybieranie zestawu dla wartości % mieszczących się w określonym przedziale",
    description:
      "wybieranie zestawu dla wartości % mieszczących się w określonym przedziale",
  };
  ...
  if (selectedGroupsByPerfectKcal.length < 1) {
    return automaticEstablishmentGroup;
  }
  ...
  console.log({ automaticEstGroups: selectedCorrectAutomaticGroups });

  if (selectedCorrectAutomaticGroups.length < 1) {
    return {
      ...automaticEstablishmentGroup,
